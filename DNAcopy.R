...
##png(paste(args[1],"Plots.png"))

###################################################
### code chunk number 2: DNAcopy.Rnw:78-79
###################################################
dataTable <- read.table(paste(args[1],"logRatioFile",sep=""),header=TRUE)
...
write.table(segment.smoothed.CNA.object[2],file=paste(args[1],"cnv.txt",sep=""))
...
sdundo.CNA.object <- segment(smoothed.CNA.object, 
                             undo.splits="sdundo", 
                             undo.SD=3,verbose=1)
write.table(sdundo.CNA.object[2],file=paste(args[1],"cnv_smoothed.txt",sep=""))

...
string additionalField[1000000];
string indel;
...
int main(int argc, char** argv) {
	...
	if (performCoverageAnalysis==1)
	{
		...
		{
			if(readsConfirmingDeletion[a]>=minimumNumberOfReadsToConfirmIndel && (collapsedDeletions[a].position2 - collapsedDeletions[a].position1)>minimumLongInsert)
			{ 
	...
	else
	{
		...
		for(a=1;a<numberOfCollapsedDeletions;a++)
		{
			...
			{
				outputDeletionsFile << collapsedDeletions[a].region << "\t" << collapsedDeletions[a].position1 << "\t"
						<< collapsedDeletions[a].position2 << "\t" << readsConfirmingDeletion[a] << "\t" << additionalField[a] << endl;
				if(generateGEFile==1) outputDeletionsFileGE << "UndefinedScaffold\t" << collapsedDeletions[a].region << "\t" << collapsedDeletions[a].position1 << "\t"
						<< collapsedDeletions[a].position2 << "\t" << (collapsedDeletions[a].position2-collapsedDeletions[a].position1) << "\t" << additionalField[a] << endl;
    ...
    cout << "Large deletion finder successfully terminated" << endl;
...
void removeTemporaryFiles(void)
{
    command = "rm " + inputDirectory + "refExtr124324*";
    system(command.c_str());
    command = "rm " + inputDirectory + "outblast.txt";
    system(command.c_str());
    command = "rm " + inputDirectory + "query.fasta";
    system(command.c_str());
    command = "rm " + inputDirectory + "unmappedEnds.txt";
    system(command.c_str());
    command = "rm " + inputDirectory + "elaboratedOutputBlast_*";
    system(command.c_str());
...
void collapseCloseDeletions(indelFormat array[1000000], int arrayLength)
{
    ...
    cout << "Converting sam file into bam......" << endl;
  
    command = "ls " + inputDirectory +"*sam | xargs -I {} -n 1 samtools view -bS {} -o {}.bam";
    //command = "./samtools view -bS " + inputDirectory + "*.sam >" + inputDirectory + "output.bam";
    cout << "Executing: " << endl;
    cout << command << endl;
    cout.flush();
    system(command.c_str());
    ...
    if (numberOfFiles>1)
    {
        command = "samtools merge " + inputDirectory + "output.bam " + inputDirectory + "*.bam";
        cout << "Executing: " << endl;
        cout << command << endl;
        cout.flush();
        system(command.c_str());
    ...
    {
        command = "mv " + inputDirectory + "*.bam " + inputDirectory + "output.bam";
        cout << "Executing: " << endl;
        cout << command << endl;
        cout.flush();
        system(command.c_str());
    ...
    command = "samtools sort -o " + inputDirectory + "output_sorted.bam " + inputDirectory + "output.bam";
    cout << "Executing: " << endl;
    cout << command << endl;
    cout.flush();
    system(command.c_str());
    ...
    cout << "Indexing the bam file....." << endl;
    command = "samtools index " + inputDirectory + "output_sorted.bam ";
    cout << "Executing: " << endl;
    cout << command << endl;
    cout.flush();
    system(command.c_str());
    ...
    cout << "Producing a copy of the reference file" << endl;
    command = "cp " + sequenceFile + " " + inputDirectory + "/refExtr124324";
    cout << "Executing: " << endl;
    cout << command << endl;
    cout.flush();
    system(command.c_str());
    ...
    cout << "Indexing reference file for blast" << endl;
    command = "./makeblastdb -in " + inputDirectory + "refExtr124324 -dbtype nucl";
    cout << "Executing: " << endl;
    cout << command << endl;
    cout.flush();
    system(command.c_str());
    ...
    for (i=1;i<numberOfCollapsedDeletions;i++)
    {
        ...
        if(readsConfirmingDeletion[i]>=minimumNumberOfReadsToConfirmIndel && (collapsedDeletions[i].position2 - collapsedDeletions[i].position1)>minimumLongInsert)
        {
            ...
            command = "samtools view " + inputDirectory + "output_sorted.bam " + collapsedDeletions[i].region + ":" + pos1a_toString + "-" + pos1b_toString + " >" + inputDirectory + "unmappedEnds.txt";
            system(command.c_str());
            
            
            
            command = "samtools view -f 4 -F 264 " + inputDirectory + "output_sorted.bam " + collapsedDeletions[i].region + ":" + pos1a_toString + "-" + pos1b_toString + " >>" + inputDirectory + "unmappedEnds.txt";
            system(command.c_str());
            
            
            
            command = "samtools view " +inputDirectory + "output_sorted.bam " + collapsedDeletions[i].region + ":" + pos2a_toString + "-" + pos2b_toString + " >>" + inputDirectory + "unmappedEnds.txt";
            system(command.c_str());
            
            
            
            command = "samtools view -f 4 -F 264 " + inputDirectory + "output_sorted.bam " + collapsedDeletions[i].region + ":" + pos2a_toString + "-" + pos2b_toString + " >>" + inputDirectory + "unmappedEnds.txt";
            system(command.c_str());
            
            filename = inputDirectory + "unmappedEnds.txt";
            unmappedReads.open(filename);
            filename = inputDirectory + "query.fasta";
            query.open(filename);
            ...
            while(!unmappedReads.eof())
            {
                ...
                numQueries += 1;
                if (numQueries>200) break;
            }
            query.close();
            
            command = "./blastn -query " + inputDirectory + "query.fasta" + " -db " + inputDirectory + "refExtr124324 -task blastn -outfmt 6 -out " + inputDirectory + "outblast.txt";
            system(command.c_str());
            ...
            filename = inputDirectory + "outblast.txt";
            blastResult.open(filename);
            
            filename = inputDirectory + "elaboratedOutputBlast_startDeletion";
            blastElaborated_startDeletion.open(filename);
            
            maxReadLength = 1;
            filename = inputDirectory + "elaboratedOutputBlast_endDeletion";
            blastElaborated_endDeletion.open(filename);
            while(!blastResult.eof())
                  {
                      ...
                      if( (queryEnd-queryStart)> maxReadLength) maxReadLength = queryEnd-queryStart;
                      ...
                      if (chromosome==collapsedDeletions[i].region && (( abs(queryStart-queryEnd)<(maxCovInBlast))) && refStart <= collapsedDeletions[i].position2 && refEnd >= (collapsedDeletions[i].position1-maxReadLength) && score>=98) //change with read length
                      {
                          
                          
                          if( refEnd < (collapsedDeletions[i].position1+1000) && refEnd>(collapsedDeletions[i].position1-1000) )
                          blastElaborated_startDeletion << chromosome << "\t" << queryStart << "\t" << queryEnd << "\t" << refStart << "\t" << refEnd << endl;
                          if( refStart > (collapsedDeletions[i].position2-1000) && refStart<(collapsedDeletions[i].position2+1000) )
            ...
            blastResult.close();
            cout << " Refined Breakpoints are " << collapsedDeletions[i].position1 << " and " << collapsedDeletions[i].position2 << " " << (collapsedDeletions[i].position2 - collapsedDeletions[i].position1) <<
            " in chromosome " << collapsedDeletions[i].region << endl;
            ...
            //get sequence ahead the deletion
            additionalField[i] = "";
            sequenceAhead = myFastaAnalyzer.getSequenceInRange(collapsedDeletions[i].region,collapsedDeletions[i].position2, (collapsedDeletions[i].position2)+200,1);
            cout << chromosome << "  " << to_string((collapsedDeletions[i].position2)+200) << endl;
            filename = inputDirectory+"query.fasta";
            sequenceAhead_fasta.open(filename);
            sequenceAhead_fasta << ">SequenceAhead" << endl << sequenceAhead << endl;
            sequenceAhead_fasta.close();
            cout << sequenceAhead << endl;
            command = "./blastn -query " + inputDirectory + "query.fasta" + " -db " + inputDirectory + "refExtr124324 -task megablast -outfmt 6 -out " + inputDirectory + "outblast2.txt";
            system(command.c_str());
            filename = inputDirectory + "outblast2.txt";
            blastResult.open(filename);
            ...
            while(!blastResult.eof())
            {
                ...
                if (chromosome==collapsedDeletions[i].region  && refStart > collapsedDeletions[i].position1 && refEnd <= (collapsedDeletions[i].position2) && score>=98 && ((refEnd-refStart)>=maxReadLength)) //change with read length
                {
                    additionalField[i] += to_string(refStart) + "\t" + to_string(score) + "\t" + to_string(refEnd-refStart) + "\t";
                }
